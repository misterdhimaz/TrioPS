...
@section('header_title', 'Update Data Konsol')

@section('content')
<div class="mb-8">
    <h2 class="font-['Orbitron',_sans-serif] text-[24px] font-bold text-white">Edit: {{ $product->name }}</h2>
    <p class="text-[#8A99B5] text-[13px]">Perbarui spesifikasi atau informasi dari konsol ini.</p>
</div>
...
<form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-[#0B1221] border border-[#1A233A] rounded-[20px] p-6 md:p-8 max-w-[1000px] shadow-sm">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $product->name) }}" required class="w-full bg-[#030712] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
        </div>

        <!-- Versi -->
        <div>
            <label class="block text-[#8A99B5] text-[11px] font-bold uppercase tracking-[1px] mb-2">Versi / Edisi</label>
            <input type="text" name="version" value="{{ old('version', $product->version) }}" required class="w-full bg-[#030712] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
        </div>
        ...
        <div>
            ...
            <div class="relative">
                <select name="category" required class="w-full bg-[#030712] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] appearance-none transition-colors">
                    <option value="PS5" {{ (old('category', $product->category) == 'PS5') ? 'selected' : '' }}>PS5</option>
                    <option value="PS4" {{ (old('category', $product->category) == 'PS4') ? 'selected' : '' }}>PS4</option>
                    <option value="Xbox" {{ (old('category', $product->category) == 'Xbox') ? 'selected' : '' }}>Xbox</option>
                    <option value="Nintendo" {{ (old('category', $product->category) == 'Nintendo') ? 'selected' : '' }}>Nintendo Switch</option>
                </select>
        ...
        <div>
            ...
            <div class="relative">
                <select name="status" required class="w-full bg-[#030712] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] appearance-none transition-colors">
                    ...
                    <option value="Limited" {{ (old('status', $product->status) == 'Limited') ? 'selected' : '' }}>Limited</option>
        ...
        <div class="md:col-span-2 border-t border-[#1A233A] pt-6">
            ...
            <input type="number" name="price_per_hour" value="{{ old('price_per_hour', $product->price_per_hour) }}" required class="w-full md:w-1/2 bg-[#030712] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
        </div>
    </div>
    ...
    <div class="bg-[#03060D] border border-[#1A233A] rounded-[16px] p-6 mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
                ...
                <input type="text" name="cpu" value="{{ old('cpu', $product->cpu) }}" required class="w-full bg-[#0B1221] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
            </div>
            <div>
                <label class="block text-[#8A99B5] text-[11px] font-bold uppercase tracking-[1px] mb-2">Resolusi</label>
                <input type="text" name="resolution" value="{{ old('resolution', $product->resolution) }}" required class="w-full bg-[#0B1221] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
            </div>
            <div>
                <label class="block text-[#8A99B5] text-[11px] font-bold uppercase tracking-[1px] mb-2">Penyimpanan (Storage)</label>
                <input type="text" name="storage" value="{{ old('storage', $product->storage) }}" required class="w-full bg-[#0B1221] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
            </div>
            <div>
                <label class="block text-[#8A99B5] text-[11px] font-bold uppercase tracking-[1px] mb-2">Konektivitas</label>
                <input type="text" name="connectivity" value="{{ old('connectivity', $product->connectivity) }}" required class="w-full bg-[#0B1221] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
            </div>
        </div>
    </div>
    ...
    <div class="mb-8">
        ...
        <input type="text" name="included_games" value="{{ old('included_games', $product->included_games) }}" required class="w-full bg-[#030712] border border-[#1A233A] text-white text-[13px] rounded-[10px] px-4 py-3 focus:outline-none focus:border-[#00e5ff] focus:ring-1 focus:ring-[#00e5ff] transition-colors">
    </div>

    <!-- Upload Gambar -->
    <div class="mb-10">
        <label class="block text-[#8A99B5] text-[11px] font-bold uppercase tracking-[1px] mb-2">Foto / Banner Konsol Saat Ini</label>

        <!-- Menampilkan gambar yang sudah ada -->
        @if($product->image)
            <div class="mb-4">
                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-40 h-24 object-cover rounded-lg border border-[#1A233A]">
            </div>
        @endif
        ...
        <div class="relative w-full bg-[#030712] border border-[#1A233A] border-dashed rounded-[10px] px-4 py-8 text-center hover:border-[#00e5ff]/50 hover:bg-[#00e5ff]/5 transition-all group">
            <!-- Perhatikan: required dihilangkan karena gambar sudah ada. User bisa pilih untuk tidak mengganti gambar -->
            ...
            <div class="text-[#8A99B5] flex flex-col items-center">
                ...
                <span class="text-[14px] font-semibold text-white mb-1">Klik atau Tarik file ke sini untuk mengganti gambar</span>
                <span class="text-[11px]">Biarkan kosong jika tidak ingin mengganti gambar saat ini.</span>
            </div>
        </div>
    </div>
