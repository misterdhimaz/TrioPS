...
<!-- MODAL DETAIL KATALOG (FIXED SCROLL & Z-INDEX) -->
<div id="modal-{{ $product->id }}"
    ...
    <!-- Modal Container: Menggunakan tinggi FIX h-[85vh] agar scroll berfungsi -->
    <div class="w-full max-w-[1100px] h-[85vh] bg-[#0B1221] border border-[#1A233A] rounded-[24px] overflow-hidden flex flex-col md:flex-row shadow-[0_0_80px_rgba(0,0,0,0.8)] relative"
        ...
        <!-- KOLOM KIRI: Visual Produk (Tinggi full, gambar menyesuaikan) -->
        ...
        <!-- KOLOM KANAN: Informasi & Form (Scroll diaktifkan) -->
        <!-- Penambahan pb-12 agar scroll bisa turun melewati tombol paling bawah -->
        <div class="w-full md:w-[55%] h-full overflow-y-auto p-6 md:p-10 pb-12 scrollbar-thin scrollbar-thumb-[#1A233A] scrollbar-track-transparent">
            ...
            <!-- Header Info -->
            <div class="mb-8 pr-12"> <!-- pr-12 agar teks tidak tertimpa tombol close -->
                <div class="inline-flex items-center gap-2 bg-[#00e5ff]/10 border border-[#00e5ff]/30 text-[#00e5ff] px-3 py-1 rounded-full text-[10px] font-bold tracking-[2px] uppercase mb-4">
            ...
            <!-- Technical Specifications Grid (PERSIS SEPERTI GAMBAR) -->
            ...
            <!-- Full Game Library Preview (FOTO GRID) -->
            <div class="mb-10">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <div class="aspect-[3/4] rounded-xl overflow-hidden border border-white/5 hover:border-[#00e5ff]/50 transition-all duration-300">
                        <img src="{{ asset('images/game1.jpg') }}" onerror="this.src='https://images.unsplash.com/photo-1605901309584-818e25960b8f?q=80&w=400&auto=format&fit=crop'" class="w-full h-full object-cover">
                    </div>
                    <div class="aspect-[3/4] rounded-xl overflow-hidden border border-white/5 hover:border-[#00e5ff]/50 transition-all duration-300">
                    ...
                    <div class="aspect-[3/4] rounded-xl overflow-hidden border border-white/5 hover:border-[#00e5ff]/50 transition-all duration-300">
                        <img src="{{ asset('images/game3.jpg') }}" onerror="this.src='https://images.unsplash.com/photo-1612287230202-1ff1d85d1bdf?q=80&w=400&auto=format&fit=crop'" class="w-full h-full object-cover">
                    </div>
                </div>
            </div>

            <!-- FORM BOOKING DINAMIS -->
            <form action="{{ route('bookings.store') }}" method="POST" id="form-booking-{{ $product->id }}" class="bg-[#03060D] border border-[#1A233A] rounded-[20px] p-6 shadow-inner">
                @csrf
                ...
                <!-- Input hidden penyimpan array jam terpilih -->
                ...
                <!-- Pick-up Time Grid (MULTI-SELECT) -->
               <!-- Pick-up Time Grid (Dirender Dinamis oleh JavaScript) -->
                <div class="mb-8">
                    ...
                    <!-- Kotak ini awalnya kosong, JS akan mengisinya -->
                    <div id="time-grid-{{ $product->id }}" class="grid grid-cols-4 gap-2">
                        <!-- Tombol jam akan muncul di sini -->
                    </div>
                </div>
